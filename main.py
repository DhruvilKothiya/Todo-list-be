...
@app.post("/tasks/")
def create_task(item : BaseTableSchema, db: Session = Depends(get_db)):
    return crud.create_task(db, user_id=item.user_id, title=item.title, is_completed=item.is_completed)

# Get all tasks for a specific user, with optional search parameter
@app.get("/users/{user_id}/tasks/")
def get_tasks(user_id: int, search: str | int = None,sort:str='asc', db: Session = Depends(get_db)):
    tasks = crud.get_tasks(db, user_id=user_id, search=search, sort=sort)
    return tasks
